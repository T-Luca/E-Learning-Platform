...
class SetsController extends Controller
{
    ...
    public function store(Request $request){
        ...
        return redirect()->action('SubcategoriesController@show', ['id' => $request->input('subcategory_id')])->with('success', 'A fost adăugat un set');
    }
            ...
            if(Auth::guest() || Auth::user()->role_id != 10){
                if((Auth::user()->role_id != 3) || (count($auth) == 0)){
                    if((Auth::user()->role_id != 2) || (Auth::user()->id != $set->user_id) || (count($auth) == 0)){
                        return redirect()->action('SubcategoriesController@show', ['id' => $set->subcategory_id])->with('error', 'Nu aveți permisiunea de a vizualiza această pagină');
                    }
                }
            }
            return view('sets.edit')->with('set', $set);    
    ...
    public function update(Request $request, $id){
        ...
        return redirect()->action('SubcategoriesController@show', ['id' => $set->subcategory_id])->with('success', 'Setul a fost editat');
    }
    ...
    public function hideCategory($id, $type){
        ...
        if($type == 'hide') {
            $set->hidden = 1;
            $set->save();
            return redirect()->back()->with('success', 'Set ascuns');
        }
        else {
            $set->hidden = 0;
            $set->save();
            return redirect()->back()->with('success', 'Setul este vizibil');
        }
    }
}
