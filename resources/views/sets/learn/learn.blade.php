...
@section('content')
    @if(!is_null($set))
        ...
        ?>
        @if($test == 1)
        <h1>Test de cunoștințe: {{ $set->name }}</h1>
        @else
        <h1>Testare: {{ $set->name }}</h1>
        @endif
        <hr>
        <div class="card">
            ...
            <div class="card-body">
                @for($i = 0; $i<count($array); $i++)
                    <div id="container{{$i}}">
                        ...
                        <div class="row">
                            <div class="col-4">
                                <button class="btn btn-primary" onclick="nextWord({{$i}})"><i class="fa fa-location-arrow" aria-hidden="true"></i> Următorul cuvânt</button>
                                <button class="btn btn-success" onclick="checkWord({{$i}})"><i class="fa fa-eye" aria-hidden="true"></i> Verificați</button>
                            </div>
                        </div>
                    </div>
                @endfor
